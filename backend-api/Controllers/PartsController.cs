...
public class PartsController : ControllerBase
{
    ...
    // PUT /api/parts/{id}
    // Updates part details (admin only).
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] UpdatePartDto dto)
    {
        ...
        // Ensure the part number isn't taken by another part
    ...
    // DELETE /api/parts/{id}
    // Deletes a part (admin only). Blocked if the part appears in any sale or purchase invoice.
    [HttpDelete("{id:int}")]
    ...
    // Returns parts where StockQty is below 10 (triggers admin notification).
    [HttpGet("low-stock")]
    public async Task<IActionResult> LowStock()
    {
        ...
        return Ok(new
        {
            totalLowStockParts = parts.Count,
            parts
        });
    }
